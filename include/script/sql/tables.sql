...
CREATE TABLE IF NOT EXISTS gold.game_evaluations (
    id SERIAL,
    game_id VARCHAR(50),
    prev_fen TEXT, 
    move VARCHAR(20),
    ...
    -- Khóa chính & Unique phải kèm theo partition key (collected_at)
    PRIMARY KEY (id, collected_at),
    UNIQUE(game_id, num_of_moves, color_turn, collected_at)
...
CREATE TABLE IF NOT EXISTS gold.games (
    ...
    -- Khóa chính & Unique phải kèm theo partition key (created_at)
...
CREATE OR REPLACE FUNCTION gold.create_game_evaluations_partitions(start_date DATE, end_date DATE)
RETURNS void AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION gold.create_games_partitions(start_date DATE, end_date DATE)
RETURNS void AS $$
DECLARE
